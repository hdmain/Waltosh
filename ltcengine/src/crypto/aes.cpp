#include "ltc/crypto/random.hpp"
#include <stdexcept>

#ifdef _WIN32
#ifndef WIN32_LEAN_AND_MEAN
#define WIN32_LEAN_AND_MEAN
#endif
#include <windows.h>
#include <bcrypt.h>
#else
#include <fstream>
#endif

namespace ltc {

Bytes random_bytes(size_t n) {
  Bytes out(n);
#ifdef _WIN32
  NTSTATUS status = BCryptGenRandom(nullptr, out.data(), ULONG(n), BCRYPT_USE_SYSTEM_PREFERRED_RNG);
  if (status != 0) throw std::runtime_error("BCryptGenRandom failed");
#else
  std::ifstream urandom("/dev/urandom", std::ios::binary);
  if (!urandom.read(reinterpret_cast<char*>(out.data()), std::streamsize(n)))
    throw std::runtime_error("urandom failed");
#endif
  return out;
}

uint32_t random_u32() {
  auto b = random_bytes(4);
  return uint32_t(b[0]) | (uint32_t(b[1]) << 8) | (uint32_t(b[2]) << 16) | (uint32_t(b[3]) << 24);
}

namespace {

constexpr uint8_t sbox[256] = {
    };

constexpr uint8_t rsbox[256] = {
    };

constexpr uint8_t rcon[11] = {0x00, 0x01, 0x02, 0x04, 0x08, 0x10, 0x20, 0x40, 0x80, 0x1b, 0x36};

uint8_t xtime(uint8_t x) { return uint8_t((x << 1) ^ ((x & 0x80) ? 0x1b : 0)); }

void key_expansion(const uint8_t* key, uint8_t round_key[240]) {
  for (int i = 0; i < 32; ++i) round_key[i] = key[i];
  int bytes = 32;
  int rcon_i = 1;
  uint8_t temp[4];
  while (bytes < 240) {
    for (int i = 0; i < 4; ++i) temp[i] = round_key[bytes - 4 + i];
    if (bytes % 32 == 0) {
      uint8_t t = temp[0];
      temp[0] = sbox[temp[1]] ^ rcon[rcon_i++];
      temp[1] = sbox[temp[2]];
      temp[2] = sbox[temp[3]];
      temp[3] = sbox[t];
    } else if (bytes % 32 == 16) {
      for (int i = 0; i < 4; ++i) temp[i] = sbox[temp[i]];
    }
    for (int i = 0; i < 4; ++i) {
      round_key[bytes] = round_key[bytes - 32] ^ temp[i];
      ++bytes;
    }
  }
}

void add_round_key(uint8_t state[16], const uint8_t* round_key, int round) {
  for (int i = 0; i < 16; ++i) state[i] ^= round_key[round * 16 + i];
}

void sub_bytes(uint8_t state[16]) {
  for (int i = 0; i < 16; ++i) state[i] = sbox[state[i]];
}
void inv_sub_bytes(uint8_t state[16]) {
  for (int i = 0; i < 16; ++i) state[i] = rsbox[state[i]];
}

void shift_rows(uint8_t state[16]) {
  uint8_t t = state[1];
  state[1] = state[5];
  state[5] = state[9];
  state[9] = state[13];
  state[13] = t;
  t = state[2];
  uint8_t t2 = state[6];
  state[2] = state[10];
  state[6] = state[14];
  state[10] = t;
  state[14] = t2;
  t = state[15];
  state[15] = state[11];
  state[11] = state[7];
  state[7] = state[3];
  state[3] = t;
}

void inv_shift_rows(uint8_t state[16]) {
  uint8_t t = state[13];
  state[13] = state[9];
  state[9] = state[5];
  state[5] = state[1];
  state[1] = t;
  t = state[2];
  uint8_t t2 = state[6];
  state[2] = state[10];
  state[6] = state[14];
  state[10] = t;
  state[14] = t2;
  t = state[3];
  state[3] = state[7];
  state[7] = state[11];
  state[11] = state[15];
  state[15] = t;
}

void mix_columns(uint8_t state[16]) {
  for (int c = 0; c < 4; ++c) {
    uint8_t* col = state + c * 4;
    uint8_t a0 = col[0], a1 = col[1], a2 = col[2], a3 = col[3];
    col[0] = uint8_t(xtime(a0) ^ xtime(a1) ^ a1 ^ a2 ^ a3);
    col[1] = uint8_t(a0 ^ xtime(a1) ^ xtime(a2) ^ a2 ^ a3);
    col[2] = uint8_t(a0 ^ a1 ^ xtime(a2) ^ xtime(a3) ^ a3);
    col[3] = uint8_t(xtime(a0) ^ a0 ^ a1 ^ a2 ^ xtime(a3));
  }
}

uint8_t mul(uint8_t a, uint8_t b) {
  uint8_t p = 0;
  for (int i = 0; i < 8; ++i) {
    if (b & 1) p ^= a;
    bool hi = a & 0x80;
    a <<= 1;
    if (hi) a ^= 0x1b;
    b >>= 1;
  }
  return p;
}

void inv_mix_columns(uint8_t state[16]) {
  for (int c = 0; c < 4; ++c) {
    uint8_t* col = state + c * 4;
    uint8_t a0 = col[0], a1 = col[1], a2 = col[2], a3 = col[3];
    col[0] = uint8_t(mul(a0, 0x0e) ^ mul(a1, 0x0b) ^ mul(a2, 0x0d) ^ mul(a3, 0x09));
    col[1] = uint8_t(mul(a0, 0x09) ^ mul(a1, 0x0e) ^ mul(a2, 0x0b) ^ mul(a3, 0x0d));
    col[2] = uint8_t(mul(a0, 0x0d) ^ mul(a1, 0x09) ^ mul(a2, 0x0e) ^ mul(a3, 0x0b));
    col[3] = uint8_t(mul(a0, 0x0b) ^ mul(a1, 0x0d) ^ mul(a2, 0x09) ^ mul(a3, 0x0e));
  }
}

void encrypt_block(uint8_t state[16], const uint8_t round_key[240]) {
  add_round_key(state, round_key, 0);
  for (int round = 1; round < 14; ++round) {
    sub_bytes(state);
    shift_rows(state);
    mix_columns(state);
    add_round_key(state, round_key, round);
  }
  sub_bytes(state);
  shift_rows(state);
  add_round_key(state, round_key, 14);
}

void decrypt_block(uint8_t state[16], const uint8_t round_key[240]) {
  add_round_key(state, round_key, 14);
  for (int round = 13; round > 0; --round) {
    inv_shift_rows(state);
    inv_sub_bytes(state);
    add_round_key(state, round_key, round);
    inv_mix_columns(state);
  }
  inv_shift_rows(state);
  inv_sub_bytes(state);
  add_round_key(state, round_key, 0);
}

}  // namespace

Bytes aes256_cbc_encrypt(const Bytes& key32, const Bytes& iv16, const Bytes& plaintext) {
  if (key32.size() != 32 || iv16.size() != 16) throw std::runtime_error("bad AES params");
  uint8_t rk[240];
  key_expansion(key32.data(), rk);
  Bytes padded = plaintext;
  size_t pad = 16 - (padded.size() % 16);
  padded.insert(padded.end(), pad, uint8_t(pad));
  Bytes out(padded.size());
  uint8_t prev[16];
  std::memcpy(prev, iv16.data(), 16);
  for (size_t i = 0; i < padded.size(); i += 16) {
    uint8_t block[16];
    for (int j = 0; j < 16; ++j) block[j] = padded[i + j] ^ prev[j];
    encrypt_block(block, rk);
    std::memcpy(out.data() + i, block, 16);
    std::memcpy(prev, block, 16);
  }
  return out;
}

Bytes aes256_cbc_decrypt(const Bytes& key32, const Bytes& iv16, const Bytes& ciphertext) {
  if (key32.size() != 32 || iv16.size() != 16 || ciphertext.size() % 16)
    throw std::runtime_error("bad AES ciphertext");
  uint8_t rk[240];
  key_expansion(key32.data(), rk);
  Bytes out(ciphertext.size());
  uint8_t prev[16];
  std::memcpy(prev, iv16.data(), 16);
  for (size_t i = 0; i < ciphertext.size(); i += 16) {
    uint8_t block[16];
    std::memcpy(block, ciphertext.data() + i, 16);
    uint8_t enc[16];
    std::memcpy(enc, block, 16);
    decrypt_block(block, rk);
    for (int j = 0; j < 16; ++j) out[i + j] = block[j] ^ prev[j];
    std::memcpy(prev, enc, 16);
  }
  if (out.empty()) throw std::runtime_error("bad padding");
  uint8_t pad = out.back();
  if (pad == 0 || pad > 16 || out.size() < pad) throw std::runtime_error("bad padding");
  for (size_t i = 0; i < pad; ++i)
    if (out[out.size() - 1 - i] != pad) throw std::runtime_error("bad padding");
  out.resize(out.size() - pad);
  return out;
}

}  // namespace ltc
